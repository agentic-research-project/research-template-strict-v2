...
- [ ] **적용 가능성**: patch가 현재 target_file에 깨끗이 적용되는가? (`git apply --check` 통과)
...
- [ ] `issues[]` 항목이 구체적인가? (막연한 "성능 개선 필요" 금지)
- [ ] Gemini가 직접 코드를 작성했는가? → ❌ 거절 (JSON review만 허용)
...
- [ ] Path C의 경우 `supporting_run_ids`에 ≥3개의 독립 실패 run이 포함됐는가?
