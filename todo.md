...
- 검사 시간 제약이 있어 과도하게 무거운 backbone이나 대형 모델은 사용하기 어렵다.
...
- source 이미지 1장당 전체 검사 시간이 1분 이내일 것
...
- reference 이미지는 결함이 없는 정상 패치 위주 (256x256, 512x512 또는 더 작은 크기 포함)
- 입력, 출력 이미지는 h,w 크기가 다를수 있음
- defect annotation은 매우 제한적이거나 없을 수 있음
- 학습은 최소화해야 하며, 가능하면 normal-only / few-shot / training-light 방식 선호
- single GPU 또는 제한된 연산 환경에서도 실행 가능해야 함
- source 이미지 1장당 검사 시간은 1분 이내여야 함
- 이미지 스타일, contrast, 노이즈 수준 변화에 대해 강건해야 함
- 지나치게 큰 foundation model end-to-end fine-tuning 방식은 지양
