...
- dataset_name: {spec.get('data_config', {}).get('dataset_name', '')}
  → data_path가 있으면 default.yaml의 data_dir에 해당 경로를 설정하고, data.py에서 해당 경로의 데이터를 로드할 것
  → data_path가 비어있거나 존재하지 않는 공개 데이터셋이면 data.py에서 자동 다운로드 구현 (torchvision 등)
...
{{
  ...
  "description": "...", "architecture_summary": "...", "param_estimate_M": 0.0
    ...
    if len(raw_response) < 10:
        print(f"  [WARN] Response too short ({len(raw_response)}), retrying...")
        raw_response = query_claude(prompt, model="claude-sonnet-4-20250514")
...
{{
  ...
  "param_estimate_M": 0.0
    ...
    return parse_json(query_claude(prompt, model="claude-sonnet-4-20250514"))
