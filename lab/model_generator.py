...
def _mechanism_audit(mechanism: str, code_files: dict, architecture_summary: str) -> dict:
    ...
    prompt = f"""당신은 딥러닝 연구 감사관입니다. 아래 mechanism이 코드에 실제 구현되었는지 판단하세요.
...
## 감사 질문
1. mechanism에서 주장하는 핵심이 실제 모델 구조/학습 절차/데이터 처리에 구현되어 있는가?
2. mechanism이 자연어 설명에만 있고 코드에는 없는가?
3. mechanism과 무관한 일반적 baseline만 구현된 것은 아닌가?

아래 JSON으로만 출력:
{{
  "implemented": true,
  "evidence": ["코드에서 mechanism 구현을 확인한 구체적 증거"],
  "missing_links": ["mechanism 중 코드에서 찾을 수 없는 요소"],
  "risk_level": "low|medium|high",
  "mechanism_mapping": {{
